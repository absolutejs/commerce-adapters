...
export const manifest = defineManifest<StripeConfig, PaymentProvider>()({
	...
	identity: {
		...
		docsUrl:
			'https://github.com/absolutejs/commerce-adapters/tree/main/stripe',
	...
	tools: {
		...
		create_coupon: tool.runtime({
			...
			input: Type.Object({
				...
				currency: Type.Optional(
					Type.String({
						description:
							'Currency for a fixed-amount coupon (default usd).'
					})
				),
				percentOff: Type.Optional(
					Type.Number({ maximum: 100, minimum: 1 })
				)
			})
		}),
		refund_order: tool.runtime({
			...
			handler: async ({ sessionId }, payments) => {
				await payments.refundBySession(sessionId);

				return `refund issued for session ${sessionId}`;
			},
			input: Type.Object({
				sessionId: Type.String({ minLength: 1 })
			})
