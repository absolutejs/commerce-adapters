...
const storeUrlIsSafe = (value: string) => {
...
export const calculateStoreEarningsCents = (input: {
	...
	const refundedSale = Math.max(
		0,
		input.saleCents - (input.refundCents ?? 0)
	);
	const base =
...
export const storeThemeIssues = (theme: StoreTheme) => {
	...
	for (const value of [theme.headerText, theme.footerText])
		if (
			...
			issues.push(
				'Header and footer text must be at most 2,000 characters.'
			);
	if (!STORE_TEMPLATE_KEYS.includes(theme.templateKey as never))
		issues.push('Choose a supported storefront template.');
	if (
		!FONT_NAME.test(theme.fonts.body) ||
		!FONT_NAME.test(theme.fonts.heading)
	)
	...
	if (theme.hero.slides !== undefined) {
		...
		if (!Array.isArray(slides) || slides.length > 7) {
			issues.push('Use at most eight homepage slides, including the primary slide.');
		} else {
			const ids = new Set<string>(['primary']);
			for (const slide of slides) {
				if (!slide || typeof slide !== 'object' || typeof slide.id !== 'string' || !slide.id.trim() || slide.id.length > 120 || ids.has(slide.id)) {
					issues.push('Homepage slides need unique identities.');
					continue;
				}
				ids.add(slide.id);
				if (typeof slide.headline !== 'string' || !slide.headline.trim() || slide.headline.length > 200) issues.push('Each slide needs a headline of at most 200 characters.');
				if (slide.subheading !== undefined && (typeof slide.subheading !== 'string' || slide.subheading.length > 1000)) issues.push('Slide descriptions must be at most 1,000 characters.');
				if (slide.imageAlt !== undefined && (typeof slide.imageAlt !== 'string' || slide.imageAlt.length > 500)) issues.push('Slide image descriptions must be at most 500 characters.');
				if (slide.imageUrl && (typeof slide.imageUrl !== 'string' || !storeUrlIsSafe(slide.imageUrl))) issues.push('Use a safe slide image URL.');
				if (slide.callToAction && (typeof slide.callToAction.label !== 'string' || !slide.callToAction.label.trim() || slide.callToAction.label.length > 120 || typeof slide.callToAction.href !== 'string' || !storeUrlIsSafe(slide.callToAction.href))) issues.push('Complete each slide button label and safe destination.');
			}
		}
	}
	...
	if (
		Object.values(theme.colors).some(
			(color) => !CSS_COLOR.test(color.trim())
		)
	)
	...
	if (
		new Set(theme.sections.map(({ id }) => id)).size !==
...
export { storeMockups, type StoreMockup } from './mockups';
