import { parseStoreMockupOverrides, type StoreMockupOverride } from "./mockups";
...
const strings = (value: unknown, label: string) => {
  ...
  return [
    ...new Set(value.map((entry) => text(entry, label, 120)).filter(Boolean)),
  ];
};
...
export const parseStoreOperatingPolicy = (
  ...
  if (
    decorators.some((provider) => !/^[a-z0-9][a-z0-9_-]{0,79}$/u.test(provider))
  )
    throw new Error(
  ...
  return {
    billing: {
      registrationCents: money(billing.registrationCents, "Registration fee"),
    ...
    coupons: {
      allowedCodes: strings(coupons.allowedCodes, "Coupon codes").map((code) =>
        code.toUpperCase(),
      ),
    ...
    shipping: {
      ...
      freeThresholdCents:
        shipping.freeThresholdCents === null
          ? null
          : money(shipping.freeThresholdCents, "Free shipping minimum"),
...
export const storeRetailPriceCents = (input: {
  ...
  if (input.marginBps !== null) {
    ...
    if (input.costCents === null)
      throw new Error("A verified cost is required for target-margin pricing");
  ...
  return (
    ...
    input.methods
      .slice(1)
      .reduce(
        (total, method) => total + (input.secondImpressionCents[method] ?? 0),
        0,
      )
  );
};
...
export type StoreProductPolicy = {
  mockupOverrides?: StoreMockupOverride[];
...
export const isManualDecorator = (
  ...
  provider === "halftone" || /^manual-[a-z0-9][a-z0-9-]{0,60}$/u.test(provider);
export const parseStoreProductPolicy = (raw: unknown): StoreProductPolicy => {
  const input = object(raw);
  return {
    ...(input.mockupOverrides !== undefined
      ? { mockupOverrides: parseStoreMockupOverrides(input.mockupOverrides) }
      : {}),
    ...(input.artworkDesignId
      ? {
          artworkDesignId: text(input.artworkDesignId, "Artwork design", 80),
        }
      : {}),
